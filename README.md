...
| **Robustness work** | Caught a seg-model out-of-distribution regression (31% different-day false alarms); multi-clip training cut it to 18%; stronger augmentation **failed** (→22%, reported as a negative); then a bigger **YOLOv8s-seg on 3 real clips (A100)** closed it to **1%** standalone (recall 0.958, with masks) — capacity + real data beat augmentation. A **det∧seg ensemble** reaches the same 1% with no retraining; an **OOD gate** defers 100% of different-day frames to review; test-time domain-normalisation **failed** too. Every lever — and every failure — is in the [report §5.8 ledger](reports/SCALEAQ_PROTOTYPE_REPORT.md). |
