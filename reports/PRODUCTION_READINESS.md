...
| Reproducibility | 8/10 | Pinned lockfile, deterministic seeds, ruff + pre-commit, CI (lint + 3.11/3.12 tests), one-command dataset/plot regeneration. Missing: full data/DVC versioning, a container-pinned build digest. |
| Serving / API robustness | 7/10 | FastAPI with path-traversal-safe access, upload size/type validation, structured logging + request IDs, `/health` `/ready` `/metrics`, a no-leak exception handler. Missing: auth, rate limiting, async batching, multi-worker scaling. |
...
| Testing | 7/10 | 48 tests: unit (geometry, metrics, data, compose, temporal), service integration (incl. security), ONNX parity helpers. Missing: coverage gating, property-based tests, load tests, a full end-to-end integration job in CI. |
...
| Security / compliance | 5/10 | Input validation, no path traversal, no stack-trace leakage, documented AGPL/CC-BY-SA provenance. Missing: auth/authz, secrets management, dependency scanning, an SBOM, threat model. |
